...
class Attention(nn.Module):
    ...
    def forward(self, x, is_human=None): 
        """ 
        will also take a binary mask where mask[i] = 1 if the i-th token overlap with the human mask and 0 otherwise
        steps:
        1 - 
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id, mask=None):
        ...
        if mask is not None:
             ...
             pooled_mask = F.avg_pool2d(mask, kernel_size=patch_size, stride=stride_size)
...
def vit_tiny_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = TransReID(img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,drop_path_rate=drop_path_rate, camera=camera, view=view, sie_xishu=sie_xishu, local_feature=local_feature,  **kwargs)
    model.in_planes = 192
    return model
